...
std::vector<miniLepton> eventNtuple::buildLeptons(bool loose) const
{
    ...
    for (unsigned int l = 0; l < mu_pt->size(); ++l) {
        ...
        if (!loose and !(mu_isfakeablesel->at(l)))
            continue;
        TLorentzVector mu;
        mu.SetPtEtaPhiE(mu_pt->at(l), mu_eta->at(l), mu_phi->at(l),
                        mu_E->at(l));
        ...
        if (mu_mcMatchType->size() > 0)
            lep.set_MCMatchType(mu_mcMatchType->at(l));
        leptons.push_back(lep);
    }

    for (unsigned int l = 0; l < ele_pt->size(); ++l) {
        // if not loose selection require to at least pass fakeable id
        if (!loose and !(ele_isfakeablesel->at(l)))
            continue;
        TLorentzVector ele;
        ele.SetPtEtaPhiE(ele_pt->at(l), ele_eta->at(l), ele_phi->at(l),
                         ele_E->at(l));
        ...
        if (ele_mcMatchType->size() > 0)
            lep.set_MCMatchType(ele_mcMatchType->at(l));
        leptons.push_back(lep);
    }
    // sort by conept
...
std::vector<miniTau> eventNtuple::buildTaus(bool loose, char WP) const
{
    ...
    for (unsigned int t = 0; t < tau_pt->size(); ++t) {
        ...
        */
        ...
        tauP4.SetPtEtaPhiE(tau_pt->at(t), tau_eta->at(t), tau_phi->at(t),
                           tau_E->at(t));
        ...
        if (WP != '-') {
            if (not tau.passMVAID(WP))
                continue;
        } else { // if WP is not explicitly set
            if (!loose and !(tau_idSelection->at(t)))
                continue;
        }
        ...
        for (unsigned int ch = 0;
             ch < (tau_signalChargedHadrCands_pt->at(t)).size(); ++ch) {
            TLorentzVector chargedhadr;
            chargedhadr.SetPtEtaPhiE(
                (tau_signalChargedHadrCands_pt->at(t))[ch],
        ...
        for (unsigned int g = 0; g < (tau_signalGammaCands_pt->at(t)).size();
             ++g) {
        ...
        for (unsigned int nh = 0;
             nh < (tau_signalNeutrHadrCands_pt->at(t)).size(); ++nh) {
...
eventNtuple::buildFourVectorTaus(std::vector<int> &decaymode, bool loose) const
{
    ...
    for (unsigned int t = 0; t < tau_pt->size(); ++t) {
        ...
        TLorentzVector tau;
        tau.SetPtEtaPhiE(tau_pt->at(t), tau_eta->at(t), tau_phi->at(t),
                         tau_E->at(t));
...
std::vector<TLorentzVector>
eventNtuple::buildFourVectorTauDaugsCharged(bool loose) const
{
...
std::vector<TLorentzVector>
eventNtuple::buildFourVectorTauDaugsNeutral(bool loose) const
{
...
std::vector<TLorentzVector> eventNtuple::buildFourVectorJets() const
{
    ...
    for (unsigned int j = 0; j < jet_pt->size(); ++j) {
        TLorentzVector jet;
        jet.SetPtEtaPhiE(jet_pt->at(j), jet_eta->at(j), jet_phi->at(j),
                         jet_E->at(j));
...
void eventNtuple::setup_branches(TTree *tree)
{
    ...
    tree->Branch("tau_byCombinedIsolationDeltaBetaCorr3Hits",
                 &tau_byCombinedIsolationDeltaBetaCorr3Hits);
    tree->Branch("tau_byLooseCombinedIsolationDeltaBetaCorr3Hits",
                 &tau_byLooseCombinedIsolationDeltaBetaCorr3Hits);
    tree->Branch("tau_byMediumCombinedIsolationDeltaBetaCorr3Hits",
                 &tau_byMediumCombinedIsolationDeltaBetaCorr3Hits);
    tree->Branch("tau_byTightCombinedIsolationDeltaBetaCorr3Hits",
                 &tau_byTightCombinedIsolationDeltaBetaCorr3Hits);
    // tree->Branch("tau_byLooseCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byLooseCombinedIsolationDeltaBetaCorr3HitsdR03);
    // tree->Branch("tau_byMediumCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byMediumCombinedIsolationDeltaBetaCorr3HitsdR03);
    // tree->Branch("tau_byTightCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byTightCombinedIsolationDeltaBetaCorr3HitsdR03);
    tree->Branch("tau_byVLooseIsolationMVArun2v1DBdR03oldDMwLT",
                 &tau_byVLooseIsolationMVArun2v1DBdR03oldDMwLT);
    tree->Branch("tau_byLooseIsolationMVArun2v1DBdR03oldDMwLT",
                 &tau_byLooseIsolationMVArun2v1DBdR03oldDMwLT);
    tree->Branch("tau_byMediumIsolationMVArun2v1DBdR03oldDMwLT",
                 &tau_byMediumIsolationMVArun2v1DBdR03oldDMwLT);
    tree->Branch("tau_byTightIsolationMVArun2v1DBdR03oldDMwLT",
                 &tau_byTightIsolationMVArun2v1DBdR03oldDMwLT);
    tree->Branch("tau_byVTightIsolationMVArun2v1DBdR03oldDMwLT",
    ...
    ///
    tree->Branch("tau_signalChargedHadrCands_pt",
                 &tau_signalChargedHadrCands_pt);
    tree->Branch("tau_signalChargedHadrCands_eta",
                 &tau_signalChargedHadrCands_eta);
    tree->Branch("tau_signalChargedHadrCands_phi",
                 &tau_signalChargedHadrCands_phi);
    tree->Branch("tau_signalChargedHadrCands_E", &tau_signalChargedHadrCands_E);
    tree->Branch("tau_signalChargedHadrCands_charge",
                 &tau_signalChargedHadrCands_charge);
...
void eventNtuple::set_branch_address(TTree *tree)
{
    ...
    tree->SetBranchAddress("btagSF_weight_HFStats1Up",
                           &btagSF_weight_HFStats1Up);
    tree->SetBranchAddress("btagSF_weight_HFStats1Down",
                           &btagSF_weight_HFStats1Down);
    tree->SetBranchAddress("btagSF_weight_HFStats2Up",
                           &btagSF_weight_HFStats2Up);
    tree->SetBranchAddress("btagSF_weight_HFStats2Down",
                           &btagSF_weight_HFStats2Down);
    tree->SetBranchAddress("btagSF_weight_LFStats1Up",
                           &btagSF_weight_LFStats1Up);
    tree->SetBranchAddress("btagSF_weight_LFStats1Down",
                           &btagSF_weight_LFStats1Down);
    tree->SetBranchAddress("btagSF_weight_LFStats2Up",
                           &btagSF_weight_LFStats2Up);
    tree->SetBranchAddress("btagSF_weight_LFStats2Down",
                           &btagSF_weight_LFStats2Down);
    ...
    tree->SetBranchAddress("ele_jetNDauChargedMVASel",
    ...
    tree->SetBranchAddress("ele_passesConversionVeto",
    ...
    tree->SetBranchAddress("tau_decayModeFindingOldDMs",
                           &tau_decayModeFindingOldDMs);
    tree->SetBranchAddress("tau_decayModeFindingNewDMs",
                           &tau_decayModeFindingNewDMs);
    tree->SetBranchAddress("tau_byCombinedIsolationDeltaBetaCorr3Hits",
                           &tau_byCombinedIsolationDeltaBetaCorr3Hits);
    tree->SetBranchAddress("tau_byLooseCombinedIsolationDeltaBetaCorr3Hits",
                           &tau_byLooseCombinedIsolationDeltaBetaCorr3Hits);
    tree->SetBranchAddress("tau_byMediumCombinedIsolationDeltaBetaCorr3Hits",
                           &tau_byMediumCombinedIsolationDeltaBetaCorr3Hits);
    tree->SetBranchAddress("tau_byTightCombinedIsolationDeltaBetaCorr3Hits",
                           &tau_byTightCombinedIsolationDeltaBetaCorr3Hits);
    // tree->SetBranchAddress("tau_byLooseCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byLooseCombinedIsolationDeltaBetaCorr3HitsdR03);
    // tree->SetBranchAddress("tau_byMediumCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byMediumCombinedIsolationDeltaBetaCorr3HitsdR03);
    // tree->SetBranchAddress("tau_byTightCombinedIsolationDeltaBetaCorr3HitsdR03",
    // &tau_byTightCombinedIsolationDeltaBetaCorr3HitsdR03);
    tree->SetBranchAddress("tau_byVLooseIsolationMVArun2v1DBdR03oldDMwLT",
                           &tau_byVLooseIsolationMVArun2v1DBdR03oldDMwLT);
    tree->SetBranchAddress("tau_byLooseIsolationMVArun2v1DBdR03oldDMwLT",
                           &tau_byLooseIsolationMVArun2v1DBdR03oldDMwLT);
    tree->SetBranchAddress("tau_byMediumIsolationMVArun2v1DBdR03oldDMwLT",
                           &tau_byMediumIsolationMVArun2v1DBdR03oldDMwLT);
    tree->SetBranchAddress("tau_byTightIsolationMVArun2v1DBdR03oldDMwLT",
                           &tau_byTightIsolationMVArun2v1DBdR03oldDMwLT);
    tree->SetBranchAddress("tau_byVTightIsolationMVArun2v1DBdR03oldDMwLT",
                           &tau_byVTightIsolationMVArun2v1DBdR03oldDMwLT);
    ...
    tree->SetBranchAddress("tau_againstElectronVLooseMVA6",
                           &tau_againstElectronVLooseMVA6);
    tree->SetBranchAddress("tau_againstElectronLooseMVA6",
                           &tau_againstElectronLooseMVA6);
    tree->SetBranchAddress("tau_againstElectronMediumMVA6",
                           &tau_againstElectronMediumMVA6);
    tree->SetBranchAddress("tau_againstElectronTightMVA6",
    ...
    tree->SetBranchAddress("tau_signalChargedHadrCands_pt",
                           &tau_signalChargedHadrCands_pt);
    tree->SetBranchAddress("tau_signalChargedHadrCands_eta",
                           &tau_signalChargedHadrCands_eta);
    tree->SetBranchAddress("tau_signalChargedHadrCands_phi",
                           &tau_signalChargedHadrCands_phi);
    tree->SetBranchAddress("tau_signalChargedHadrCands_E",
                           &tau_signalChargedHadrCands_E);
    tree->SetBranchAddress("tau_signalChargedHadrCands_charge",
                           &tau_signalChargedHadrCands_charge);
    tree->SetBranchAddress("tau_signalNeutrHadrCands_pt",
                           &tau_signalNeutrHadrCands_pt);
    tree->SetBranchAddress("tau_signalNeutrHadrCands_eta",
                           &tau_signalNeutrHadrCands_eta);
    tree->SetBranchAddress("tau_signalNeutrHadrCands_phi",
                           &tau_signalNeutrHadrCands_phi);
    tree->SetBranchAddress("tau_signalNeutrHadrCands_E",
                           &tau_signalNeutrHadrCands_E);
    tree->SetBranchAddress("tau_signalGammaCands_pt", &tau_signalGammaCands_pt);
    tree->SetBranchAddress("tau_signalGammaCands_eta",
                           &tau_signalGammaCands_eta);
    tree->SetBranchAddress("tau_signalGammaCands_phi",
                           &tau_signalGammaCands_phi);
