...
// constructor
// MVAVars::MVAVars(const std::vector<miniLepton>& leptons, const
// std::vector<TLorentzVector>& taus, const std::vector<TLorentzVector>& jets,
// float met, float metphi, float mht)
//{
//	compute_all_variables(leptons, taus, jets, met, metphi, mht);
//}

void MVAVars::compute_all_variables(const std::vector<miniLepton> &leptons,
                                    ...
                                    int ntags_loose)
{
    ...
    met_ = met;
    mT_met_lep0_ = sqrt(2 * leptons[0].conept() * met *
                        (1 - cos(leptons[0].phi() - metphi)));
    ...
    if (anaType_ == Analyze_2lss1tau) {
        ...
        mindr_lep1_jet_ =
            (leptons.size() > 1) ? compute_mindr(leptons[1].p4(), jets) : -9999;

        max_lep_eta_ = (leptons.size() > 1) ? std::max(fabs(leptons[0].eta()),
                                                       fabs(leptons[1].eta()))
                                            : -9999.;
        ...
        dr_leps_ = (leptons.size() > 1)
                       ? leptons[0].p4().DeltaR(leptons[1].p4())
                       : -9999.;

        tau_pt_ = taus[0].pt();

        dr_lep0_tau_ = leptons[0].p4().DeltaR(taus[0].p4());
        dr_lep1_tau_ = (leptons.size() > 1)
                           ? leptons[1].p4().DeltaR(taus[0].p4())
                           : -9999.;

        mvis_lep0_tau_ = (leptons[0].p4() + taus[0].p4()).M();
        mvis_lep1_tau_ = (leptons.size() > 1)
                             ? (leptons[1].p4() + taus[0].p4()).M()
                             : -9999.;
        ...
        dr_lep_tau_ss_ = (leptons[0].charge() == taus[0].charge())
                             ? dr_lep_tau0_
                             : dr_lep_tau1_;
        ...
        max_lep_eta_ =
            std::max(std::max(fabs(leptons[0].eta()), fabs(leptons[1].eta())),
                     fabs(leptons[2].eta()));
...
void MVAVars::compute_taudecay_variables(
    const std::vector<TLorentzVector> &taus,
    ...
    const std::vector<int> decaymode)
{
    ...
    tau0_upsilon_ = (chargedDaug[0].E() - neutralDaug[0].E()) /
                    (chargedDaug[0].E() + neutralDaug[0].E());

    if (anaType_ == Analyze_1l2tau) {
        assert(taus.size() > 1);
        tau1_decaymode_ = decaymode[1];
        tau1_energy_ = taus[1].E();
        tau1_upsilon_ = (chargedDaug[1].E() - neutralDaug[1].E()) /
                        (chargedDaug[1].E() + neutralDaug[1].E());
    }
}
...
void MVAVars::compute_taudecay_variables(const std::vector<miniTau> &taus)
{
    ...
    if (anaType_ == Analyze_1l2tau) {
        ...
        // taup_upsilon_ =
        // (taus[ip].chargedDaughtersP4().E()-taus[ip].neutralDaughtersP4().E())
        // /
        // (taus[ip].chargedDaughtersP4().E()+taus[ip].neutralDaughtersP4().E());
        ...
        // taum_upsilon_ =
        // (taus[im].chargedDaughtersP4().E()-taus[im].neutralDaughtersP4().E())
        // /
        // (taus[im].chargedDaughtersP4().E()+taus[im].neutralDaughtersP4().E());
    } else {
        ...
        // tau0_upsilon_ =
        // (taus[0].chargedDaughtersP4().E()-taus[0].neutralDaughtersP4().E()) /
        // (taus[0].chargedDaughtersP4().E()+taus[0].neutralDaughtersP4().E());
    }
}
...
/*
void MVAVars::compute_all_variables(const std::vector<miniLepton>& leptons,
const std::vector<miniTau>& taus, const std::vector<TLorentzVector>& jets, float
met, float metphi, float mht, int ntags_loose)
{
...
float MVAVars::compute_mindr(const TLorentzVector &l,
...
float MVAVars::compute_cosPsi(const TLorentzVector &p1,
...
{
    const TString datadir = (std::string(getenv("CMSSW_BASE")) +
    ...
    if (anaType_ == Analyze_2lss1tau) {
        /// ttV
        const TString weights_ttV =
            datadir + "tthMVA/2lss1tau_v2/mvaTTHvsTTV2lss1tau_BDTG.weights.xml";
        ...
        //// ttbar
        const TString weights_ttbar =
            datadir +
            "tthMVA/2lss1tau_v2/mvaTTHvsTTbar2lss1tau_BDTG.weights.xml";
        ...
        /// ttV
        const TString weights_ttV =
            datadir + "evtMVAWeights/1l2tau/sklearn_ttZ.xml";
        reader_ttV = new TMVA::Reader("!Color:!Silent");
        ...
        /// ttbar
        const TString weights_ttbar =
            datadir + "evtMVAWeights/1l2tau/sklearn_tt.xml";
        reader_ttbar = new TMVA::Reader("!Color:!Silent");
        ...
        /// ttV
        const TString weights_ttV =
            datadir + "evtMVAWeights/3l1tau/3l_ttV_BDTG.weights.xml";
        reader_ttV = new TMVA::Reader("!Color:!Silent");
        ...
        /// ttbar
        const TString weights_ttbar =
            datadir + "evtMVAWeights/3l1tau/3l_ttbar_BDTG.weights.xml";
        reader_ttbar = new TMVA::Reader("!Color:!Silent");
...
float MVAVars::BDT_ttV() { return reader_ttV->EvaluateMVA("BDT"); }

float MVAVars::BDT_ttbar() { return reader_ttbar->EvaluateMVA("BDT"); }
