    ...
    const float metLD)
{
    ...
    if (evt_selector_->pass_tight_charge(lep_fakeable)) {
        if (doCutflow_)
            fill_CutFlow(ibin++, "tight charge");
    } else
        return false;
    ...
    */
    ...
    // for control region, the lepton that has the same sign as tau has to be a
    // electron, and the charge flip rate is only applied to this electron
    // To save computing time, additional requirement on tau charge applied
    // after ntuple production for signal or control region
...
bool ttHtautauAnalyzer::pass_event_sel_1l2tau(
    ...
    const std::vector<pat::Tau> &taus, const int njets, const int nbtags_loose,
    const int nbtags_medium)
{
    if (debug_)
    ...
    if (pass_taupair_charge) {
        ...
        return false;
    */
    // Requirement on tau charge is applied after ntuple production for signal
    // or control region
...
{
    if (debug_)
        std::cout << "start event selection: 3l1tau" << std::endl;
