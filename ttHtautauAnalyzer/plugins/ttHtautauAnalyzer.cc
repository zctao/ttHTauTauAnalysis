...
//
/**\class ttHtautauAnalyzer ttHtautauAnalyzer.cc
 ttHTauTauAnalysis/ttHtautauAnalyzer/plugins/ttHtautauAnalyzer.cc
      ...
      csv_tight_wp_(iConfig.getParameter<double>("csv_tight_wp"))
{
    ...
    trigger_token_ = consumes<edm::TriggerResults>(
        edm::InputTag("TriggerResults", "",
                      iConfig.getParameter<std::string>("HLT_config_tag")));
    filter_token_ = consumes<edm::TriggerResults>(
        edm::InputTag("TriggerResults", "",
                      iConfig.getParameter<std::string>("filter_config_tag")));
    vtx_token_ = consumes<reco::VertexCollection>(
        iConfig.getParameter<edm::InputTag>("pv"));
    pu_token_ = consumes<std::vector<PileupSummaryInfo>>(
        iConfig.getParameter<edm::InputTag>("pileup"));
    bs_token_ = consumes<reco::BeamSpot>(
        iConfig.getParameter<edm::InputTag>("beamspot"));
    rho_token_ = consumes<double>(iConfig.getParameter<edm::InputTag>("rho"));
    electrons_token_ = consumes<pat::ElectronCollection>(
        iConfig.getParameter<edm::InputTag>("electrons"));
    muons_token_ = consumes<pat::MuonCollection>(
        iConfig.getParameter<edm::InputTag>("muons"));
    taus_token_ = consumes<pat::TauCollection>(
        iConfig.getParameter<edm::InputTag>("taus"));
    jets_token_ = consumes<pat::JetCollection>(
        iConfig.getParameter<edm::InputTag>("jets"));
    mets_token_ = consumes<pat::METCollection>(
        iConfig.getParameter<edm::InputTag>("mets"));
    genparticle_token_ = consumes<reco::GenParticleCollection>(
        iConfig.getParameter<edm::InputTag>("prunedgen"));
    genjets_token_ =
        consumes<reco::GenJetCollection>(edm::InputTag("slimmedGenJets"));

    // badMuons_token_ =
    // consumes<edm::View<reco::Muon>>(iConfig.getParameter<edm::InputTag>("badmu"));
    // clonedMuons_token_ =
    // consumes<edm::View<reco::Muon>>(iConfig.getParameter<edm::InputTag>("clonemu"));
...
void ttHtautauAnalyzer::analyze(const edm::Event &iEvent,
                                const edm::EventSetup &iSetup)
{
    ...
    if (not isdata_) {
        ...
        for (PVI = PU_info->begin(); PVI != PU_info->end(); ++PVI) {
            ...
            if (BX == 0) { // "0" is the in-time crossing, negative values are
                           // the early crossings, positive are late
    ...
    if (debug_) {
        std::cout << "n_electrons_loose : " << ele_preselected.size()
                  << std::endl;
    ...
    // tauES
    std::vector<pat::Tau> tau_corrected =
        getCorrectedTaus(*taus, tauES_unc_, TESType_);
    ...
    if (isdata_) {
        ...
        const JetCorrectorParameters &JetCorPar =
            (*JetCorParColl)["Uncertainty"];
    ...
    // remove overlap
    std::vector<pat::Jet> jet_no_lep =
        removeOverlapdR(jet_raw, lep_fakeable, 0.4);
    std::vector<pat::Jet> jet_cleaned =
        removeOverlapdR(jet_no_lep, tau_loose, 0.4);
    ...
    float MET = pfMET.pt();
    // MHT
    // float MHT = getMHT(mu_fakeable, ele_fakeable, tau_selected,
    // jet_selected);
    float MHT = getMHT(lep_loose, tau_loose, jet_selected);
    ...
    if (anaType_ == Analyze_2lss1tau) {
        ...
        pass_event_sel = evt_selector_->pass_2l1tau_inclusive_selection(
            ...
            jet_selected.size(), n_btags_loose, n_btags_medium, metLD,
            h_CutFlow_);
...
// ------------
void ttHtautauAnalyzer::beginJob()
{
...
// ------------
void ttHtautauAnalyzer::endJob() {}

// ------------ method called when starting to processes a run  ------------
void ttHtautauAnalyzer::beginRun(const edm::Run &iRun,
                                 const edm::EventSetup &iSetup)
{
...
// ------------
/*
void ttHtautauAnalyzer::endLuminosityBlock(const edm::LuminosityBlock & lumi,
...
// module  ------------
void ttHtautauAnalyzer::fillDescriptions(
