...
{
    evNtuple_.bTagSF_weight = sf_helper_->Get_EvtCSVWeight(jets, "NA");
    evNtuple_.btagSF_weight_LFUp = sf_helper_->Get_EvtCSVWeight(jets, "LFUp");
    evNtuple_.btagSF_weight_LFDown =
        sf_helper_->Get_EvtCSVWeight(jets, "LFDown");
    evNtuple_.btagSF_weight_HFUp = sf_helper_->Get_EvtCSVWeight(jets, "HFUp");
...
void ttHtautauAnalyzer::write_ntuple_leptonSF(
...
void ttHtautauAnalyzer::write_ntuple_triggerSF(
    const miniLepton &lep, const std::vector<pat::Tau> &taus, bool LTriggered,
    bool XTriggered)
{
...
void ttHtautauAnalyzer::write_ntuple_frweight(
    const std::vector<miniLepton> &leps, const std::vector<pat::Tau> &taus)
{
...
void ttHtautauAnalyzer::write_ntuple_frweight(
    const std::vector<miniLepton> &leps, const std::vector<miniTau> &taus)
{
    evNtuple_.FR_weight = sf_helper_->Get_FR_weight(leps, taus);
}
...
void ttHtautauAnalyzer::write_ntuple_muons(const std::vector<pat::Muon> &muons)
{
    for (auto &mu : muons) {
        ...
        evNtuple_.mu_jetNDauChargedMVASel->push_back(
            mu.userFloat("nearestJetNDauCharged"));
        evNtuple_.mu_miniRelIso->push_back(mu.userFloat("miniRelIso"));
        evNtuple_.mu_miniIsoCharged->push_back(
            mu.userFloat("miniAbsIsoCharged"));
        evNtuple_.mu_miniIsoNeutral->push_back(
            mu.userFloat("miniAbsIsoNeutralcorr"));
...
void ttHtautauAnalyzer::write_ntuple_electrons(
    const std::vector<pat::Electron> &electrons)
{
    for (auto &ele : electrons) {
        ...
        evNtuple_.ele_jetNDauChargedMVASel->push_back(
            ele.userFloat("nearestJetNDauCharged"));
        evNtuple_.ele_miniRelIso->push_back(ele.userFloat("miniRelIso"));
        evNtuple_.ele_miniIsoCharged->push_back(
            ele.userFloat("miniAbsIsoCharged"));
        evNtuple_.ele_miniIsoNeutral->push_back(
            ele.userFloat("miniAbsIsoNeutralcorr"));
        ...
        evNtuple_.ele_isChargeConsistent->push_back(
            ele.isGsfCtfScPixChargeConsistent() +
                ele.isGsfScPixChargeConsistent() >
            1);
...
void ttHtautauAnalyzer::write_ntuple_taus(const std::vector<pat::Tau> &taus)
{
    for (auto &tau : taus) {
        ...
        evNtuple_.tau_decayModeFindingOldDMs->push_back(
            tau.tauID("decayModeFinding"));
        evNtuple_.tau_decayModeFindingNewDMs->push_back(
            tau.tauID("decayModeFindingNewDMs"));
        evNtuple_.tau_byCombinedIsolationDeltaBetaCorr3Hits->push_back(
            tau.tauID("byCombinedIsolationDeltaBetaCorrRaw3Hits"));
        evNtuple_.tau_byLooseCombinedIsolationDeltaBetaCorr3Hits->push_back(
            tau.tauID("byLooseCombinedIsolationDeltaBetaCorr3Hits"));
        evNtuple_.tau_byMediumCombinedIsolationDeltaBetaCorr3Hits->push_back(
            tau.tauID("byMediumCombinedIsolationDeltaBetaCorr3Hits"));
        evNtuple_.tau_byTightCombinedIsolationDeltaBetaCorr3Hits->push_back(
        ...
        evNtuple_.tau_byVLooseIsolationMVArun2v1DBdR03oldDMwLT->push_back(
            tau.tauID("byVLooseIsolationMVArun2v1DBdR03oldDMwLT"));
        evNtuple_.tau_byLooseIsolationMVArun2v1DBdR03oldDMwLT->push_back(
            tau.tauID("byLooseIsolationMVArun2v1DBdR03oldDMwLT"));
        evNtuple_.tau_byMediumIsolationMVArun2v1DBdR03oldDMwLT->push_back(
            tau.tauID("byMediumIsolationMVArun2v1DBdR03oldDMwLT"));
        evNtuple_.tau_byTightIsolationMVArun2v1DBdR03oldDMwLT->push_back(
            tau.tauID("byTightIsolationMVArun2v1DBdR03oldDMwLT"));
        evNtuple_.tau_byVTightIsolationMVArun2v1DBdR03oldDMwLT->push_back(
            tau.tauID("byVTightIsolationMVArun2v1DBdR03oldDMwLT"));
        evNtuple_.tau_againstMuonLoose3->push_back(
            tau.tauID("againstMuonLoose3"));
        evNtuple_.tau_againstMuonTight3->push_back(
            tau.tauID("againstMuonTight3"));
        evNtuple_.tau_againstElectronVLooseMVA6->push_back(
            tau.tauID("againstElectronVLooseMVA6"));
        evNtuple_.tau_againstElectronLooseMVA6->push_back(
            tau.tauID("againstElectronLooseMVA6"));
        evNtuple_.tau_againstElectronMediumMVA6->push_back(
            tau.tauID("againstElectronMediumMVA6"));
        evNtuple_.tau_againstElectronTightMVA6->push_back(
            tau.tauID("againstElectronTightMVA6"));
