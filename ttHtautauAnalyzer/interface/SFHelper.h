...
class SFHelper
{
  public:
    ...
    // member functions
    float Get_HLTSF(const std::vector<miniLepton> &,
                    const std::vector<miniTau> &, bool, bool);
    float Get_HLTSF_2l1tau(int);
    float Get_HLTSF_3l1tau();
    float Get_HLTSF_1l2tau(const miniLepton &, const std::vector<miniTau> &,
                           bool, bool);
    float Get_HLTSF_1l2tau(float, float, int, float, float, int, float, float,
                           int, bool, bool);
#if !defined(__ACLIC__) && !defined(__ROOTCLING__)
    float Get_HLTSF_1l2tau(const miniLepton &, const std::vector<pat::Tau> &,
                           bool, bool);
#endif
